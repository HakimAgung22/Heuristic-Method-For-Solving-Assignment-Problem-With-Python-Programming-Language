...
#********************** Method *******************/
def Heuristic(mat,tM):
    lMat=[]
    row = len(mat)
    col = len(mat[0])
    lp = CountAllPinalty(mat,0,tM,"heuristic")
    i = 0
    l = []
    ...
    i=1
    while True:
        key = GetPinalty(lp,mat,0,"heuristic")
        lp = PopListPinalty(lp,key)
        mat,val,key = ReductionMatrix(mat,key,tM)
        lp = PopListPinalty(lp,key)
        if(len(mat) !=1):
            lp = UpdatePinalty(mat,lp,tM)
        print("_____________________________________________")
        print("******************ITERASI "+str(i+1)+"******************")
        PrintMatrix(mat)
        l.append(val)
...
if __name__ == "__main__":
    ...
    while True:
        print("MASALAH PENUGASAN MENGGUNAKAN HEURISTIC METHOD")
        print("  Pilih kasus : ")
        ...
        if(choose == 1):
            result = Heuristic(mat,"min")
        elif(choose == 2):
            result = Heuristic(mat,"max")    
        else:
